    ...
    public function actionSteamLogin() {
        ...
        try {
            ...
            if (!$openId->mode) {
                ...
                if (isset($matches['user_id'])) {
                    ...
                    if (isset($userData->response, $userData->response->players, $userData->response->players[0], $userData->response->players[0]->steamid)) {
                        ...
                        if (($user = User::find()->where(['steam_id' => $userData->response->players[0]->steamid])->one()) === null) {
                            ...
                            $userInfoFlag = false;
                            $checkUsername = User::find()->where(['username' => $userData->response->players[0]->personaname])->one();

                            $createdUsername = $userData->response->players[0]->personaname;

                            if (!is_null($checkUsername)) {
                                $userInfoFlag = true;
                                $createdUsername = $createdUsername . '(' . \Yii::$app->security->generateRandomString($length = 5) . ')';
                            }
                            ...
                            if ($user->save()) {
                                ...
                                if ($userInfoFlag) {
                                    Yii::$app->session->setFlash('info', 'Добвка к вашему имени в круглых скобках обозначает что пользователь с таким именем существует, оставьте как есть либо придумайте другое имя');
                                }
                ...
                Yii::$app->session->setFlash('error', Yii::t('app', 'User has canceled authentication!'));
            }
        } catch (Exception $ex) {
            Yii::$app->session->setFlash('error', Yii::t('app', 'User has canceled authentication!'));
        }
